...
error_new(js_val *instance, js_args *args, eval_state *state)
{
  ...
  js_set_class(err, "Error");

  if (!IS_UNDEF(msg))
    js_set(err, "message", TO_STR(msg));

  err->proto = js_try_get_proto("Error");
  return err;
}
...
error_eval_new(js_val *instance, js_args *args, eval_state *state)
{
  ...
  js_set(err, "name", JSSTR(E_EVAL));
  err->proto = js_try_get_proto(E_EVAL);
  return err;
}
...
{
  js_val *err = error_new(instance, args, state);
  js_set(err, "name", JSSTR(E_RANGE));
  err->proto = js_try_get_proto(E_RANGE);
  return err;
}
...
{
  js_val *err = error_new(instance, args, state);
  js_set(err, "name", JSSTR(E_REFERENCE));
  err->proto = js_try_get_proto(E_REFERENCE);
  return err;
}
...
{
  js_val *err = error_new(instance, args, state);
  js_set(err, "name", JSSTR(E_SYNTAX));
  err->proto = js_try_get_proto(E_SYNTAX);
  return err;
}
...
{
  js_val *err = error_new(instance, args, state);
  js_set(err, "name", JSSTR(E_TYPE));
  err->proto = js_try_get_proto(E_TYPE);
  return err;
}
...
{
  js_val *err = error_new(instance, args, state);
  js_set(err, "name", JSSTR(E_URI));
  err->proto = js_try_get_proto(E_URI);
  return err;
}
...
error_proto_to_string(js_val *instance, js_args *args, eval_state *state)
{
  if (!IS_OBJ(instance))
    js_throw(state,
        js_new_error(E_TYPE, "Error.prototype.toString called on non-object"));

  js_val *name_prop = js_get_proto(instance, "name");
  js_val *msg_prop = js_get_proto(instance, "message");
  ...
  return JSSTR(js_str_concat(js_str_concat(name->string.ptr, ": "), msg->string.ptr));
}
