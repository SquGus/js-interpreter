...
{
  js_val *info = JSOBJ();
  js_set_prop(info, "arenas", JSNUM(js->gc_num_arenas), P_DEFAULT);
  js_set_prop(info, "arenaSize", JSNUM(SLOTS_PER_ARENA), P_DEFAULT);
  js_set_prop(info, "runs", JSNUM(js->gc_runs), P_DEFAULT);
  js_set_prop(info, "lastStart", JSNUM(js->gc_last_start), P_DEFAULT);
  js_set_prop(info, "time", JSNUM(js->gc_time), P_DEFAULT);
  return info;
}
...
bootstrap_gc()
{
  ...
  js_attach_prototype(gc, js->function_proto);
