...
{
  js_throw(state, js_new_error(E_ERROR, "Unicode is not supported"));
  UNREACHABLE();
}
...
str_proto_concat(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (i = 0; i < ARGLEN(args); i++) {
    ...
    tmp = new->string.ptr;
    new->string.ptr = js_str_concat(new->string.ptr, arg->string.ptr);
...
str_proto_match(js_val *instance, js_args *args, eval_state *state)
{
  ...
  if (!IS_REGEXP(regexp))
    regexp = js_new_regexp("");

  js_args *exec_args;

  bool global = js_get_proto(regexp, "global")->boolean.val;
  if (!global) {
    exec_args = args_new();
  ...
  js_set(regexp, "lastIndex", JSNUM(0));
  ...
  while (last_match) {
    ...
    this_ind = TO_NUM(js_get(regexp, "lastIndex"))->number.val;
    if (this_ind == prev_last_ind) {
      js_set(regexp, "lastIndex", JSNUM(this_ind + 1));
      prev_last_ind = this_ind + 1;
    }
    else {
      prev_last_ind = this_ind;
    }
    match_str = js_get(result, "0");
    js_set(arr, JSNUMKEY(n)->string.ptr, match_str);
    n++;
  }
...
splice(char *str, char *rep, int start, int end)
{
  ...
  front = js_str_slice(str, 0, start);
  back = js_str_slice(str, end, strlen(str));
  tmp = js_str_concat(front, rep);
  new = js_str_concat(tmp, back);
  free(front);
  free(back);
  free(tmp);
  return new;
...
str_proto_replace(js_val *instance, js_args *args, eval_state *state)
{
  ...
  if (!IS_REGEXP(search_val)) {
    ...
    return JSSTR(js_str_replace(str, search, replace, 1));
  }

  bool global = TO_BOOL(js_get_proto(search_val, "global"))->boolean.val,
       caseless = TO_BOOL(js_get_proto(search_val, "ignoreCase"))->boolean.val;

  char *pattern = js_get(search_val, "source")->string.ptr;
  char *repl = TO_STR(replace_val)->string.ptr;
  int count, *matches;

  js_set(search_val, "lastIndex", JSNUM(0));

  unsigned i = 0;
  while (i < strlen(str)) {
    matches = js_regexp(str, pattern, &count, i, caseless);
    if (count == 0) break;
    js_set(search_val, "lastIndex", JSNUM(matches[1]));
...
str_proto_search(js_val *instance, js_args *args, eval_state *state)
{
  ...
  if (!IS_REGEXP(regexp)) {
    ...
    regexp = JSRE("");
    js_set(regexp, "source", tmp);
  }

  char *pattern = js_get(regexp, "source")->string.ptr;
  bool caseless = js_get_proto(regexp, "ignoreCase")->boolean.val;
  int count, *matches = js_regexp(str, pattern, &count, 0, caseless);
...
str_proto_slice(js_val *instance, js_args *args, eval_state *state)
{
  ...
  return JSSTR(js_str_slice(instance->string.ptr, start, end));
}
...
regexp_splitter(char *str, js_val *regexp, int limit)
{
  ...
  char *source = TO_STR(js_get_proto(regexp, "source"))->string.ptr;
  bool caseless = TO_BOOL(js_get_proto(regexp, "ignoreCase"))->boolean.val;
  // int ncaps = js_regexp_ncaptures(source);
  js_val *arr = JSARR();
  int count, *matches;
  char *tmp;
  unsigned i, j;
  ...
  for (i = 0, j = 0; i < strlen(str); j++) {
    matched_last = false;
    matches = js_regexp(str, source, &count, i, caseless);
    if (count == 0) break;
    tmp = js_str_slice(str, i, matches[0]);
    js_set(arr, JSNUMKEY(j)->string.ptr, JSSTR(tmp));
    i = matches[1];
    free(matches);
    matched_last = true;
  }
  ...
  if (i < strlen(str)) {
    tmp = js_str_slice(str, i, strlen(str));
    js_set(arr, JSNUMKEY(j++)->string.ptr, JSSTR(tmp));
  }
  else if (matched_last)
    js_set(arr, JSNUMKEY(j++)->string.ptr, JSSTR(""));
...
str_proto_split(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (match == (int)strlen(sep)) {
      split = js_str_slice(str, start, i - strlen(sep) + 1);
      js_set(arr, JSNUMKEY(index++)->string.ptr, JSSTR(split));
      free(split);
      start = i + 1;
      match = 0;
      matched_last = true;
  ...
  if (limit > 0) {
    if (start != len) {
      split = js_str_slice(str, start, len);
      js_set(arr, JSNUMKEY(index++)->string.ptr, JSSTR(split));
      free(split);
    }
    else if (matched_last && strlen(sep))
      js_set(arr, JSNUMKEY(index++)->string.ptr, JSSTR(""));
  }

  js_set_len(arr, index);
  return arr;
}
...
str_proto_substring(js_val *instance, js_args *args, eval_state *state)
{
  ...
  return JSSTR(js_str_slice(instance->string.ptr, from, to));
}
