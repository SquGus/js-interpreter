  ...
  if (IS_OBJ(props)) {
    ...
    OBJ_ITER(props, p) {
      ...
      if (p->ptr && IS_OBJ(p->ptr)) {
        ...
        js_set_prop(obj, p->name, js_get(p->ptr, "value"), flags);
      }
    }
  }
...
obj_define_property(js_val *instance, js_args *args, eval_state *state)
{
  ...
  js_set_prop(obj, prop->string.ptr, js_get(desc, "value"), flags);
  return obj;
}
...
obj_get_own_property_descriptor(js_val *instance, js_args *args, eval_state *state)
{
  ...
  js_prop *prop = js_get_prop(obj, prop_name->string.ptr);
  js_val *descriptor = JSOBJ();

  js_set(descriptor, "value", prop->ptr);
  js_set(descriptor, "configurable", JSBOOL(prop->configurable));
  js_set(descriptor, "writable", JSBOOL(prop->writable));
  js_set(descriptor, "enumerable", JSBOOL(prop->enumerable));
...
obj_keys(js_val *instance, js_args *args, eval_state *state)
{
  ...
  OBJ_ITER(obj, p) {
    if (p->enumerable)
      js_set(keys, JSNUMKEY(i++)->string.ptr, JSSTR(p->name));
  }
...
obj_get_own_property_names(js_val *instance, js_args *args, eval_state *state)
{
  ...
  int i = 0;
  OBJ_ITER(obj, p) {
    js_set(names, JSNUMKEY(i++)->string.ptr, JSSTR(p->name));
  }
...
obj_proto_has_own_property(js_val *instance, js_args *args, eval_state *state)
{
  ...
  return JSBOOL(js_get_prop(instance, prop_name->string.ptr) != NULL);
}
...
{
  js_val *prop_name = ARG(args, 0);
  js_prop *prop = js_get_prop(instance, prop_name->string.ptr);
...
obj_proto_to_string(js_val *instance, js_args *args, eval_state *state)
{
  ...
  return JSSTR(js_str_concat(js_str_concat("[object ", class), "]"));
}
...
obj_or_throw(js_val *maybe_obj, eval_state *state, const char *name)
{
  if (!maybe_obj || !IS_OBJ(maybe_obj))
    js_throw(state, js_new_error(E_TYPE, "Object.%s called on a non-object", name));
  return maybe_obj;
}
...
flags_from_descriptor(js_val *desc)
{
  ...
  js_val *enumerable = js_get(desc, "enumerable");
  js_val *configurable = js_get(desc, "configurable");
  js_val *writable = js_get(desc, "writable");
