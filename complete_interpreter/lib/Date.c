...
static double in_leap_year(double t);
...
date_format_loc(double ut, bool incl_date, bool incl_time)
{
  ...
  if (incl_date) {
    ...
    res->string.ptr = js_str_concat(res->string.ptr, date);
  }

  if (incl_date && incl_time)
    res->string.ptr = js_str_concat(res->string.ptr, " ");
  ...
  if (incl_time) {
    ...
    res->string.ptr = js_str_concat(res->string.ptr, time);
  }
  return res;
}
...
date_parse_iso(char *str, double *t)
{
  ...
  // Pull out the parts
  int year = atoi(js_str_slice(str, 0, 4));
  int month = atoi(js_str_slice(str, 5, 7)) - 1;
  int date = atoi(js_str_slice(str, 8, 10));
  int hour = atoi(js_str_slice(str, 11, 13));
  int min = atoi(js_str_slice(str, 14, 16));
  int sec = atoi(js_str_slice(str, 17, 19));
  int msec = atoi(js_str_slice(str, 20, 23));
...
date_parse_loc(char *str, double *t)
{
  ...
  char *mname = js_str_slice(str, 4, 7);
  int date = atoi(js_str_slice(str, 8, 10));
  int year = atoi(js_str_slice(str, 11, 15));
  int hour = atoi(js_str_slice(str, 16, 18));
  int min = atoi(js_str_slice(str, 19, 21));
  int sec = atoi(js_str_slice(str, 22, 24));
...
date_parse_utc(char *str, double *t)
{
  ...
  // Pull out the parts
  int date = atoi(js_str_slice(str, 5, 7));
  char *mname = js_str_slice(str, 8, 11);
  int year = atoi(js_str_slice(str, 12, 16));
  int hour = atoi(js_str_slice(str, 17, 19));
  int min = atoi(js_str_slice(str, 20, 22));
  int sec = atoi(js_str_slice(str, 23, 25));
