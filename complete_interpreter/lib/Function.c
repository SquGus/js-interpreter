...
func_new(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (i = 0; arglen > 0 && i < (arglen - 1); i++) {
    if (!arg_lst) {
      ...
      tmp = arg_lst;
      arg_lst = js_str_concat(arg_lst, ", ");
      free(tmp);
      tmp = arg_lst;
      arg_lst = js_str_concat(arg_lst, TO_STR(ARG(args, i))->string.ptr);
      free(tmp);
    }
  }
  ...
  return js_eval_string(func_def, state->ctx);
}
...
func_proto_apply(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (i = 0; i < arr->object.length; i++)
    args_append(func_args, js_get(arr, JSNUMKEY(i)->string.ptr));

  return js_call(state->ctx, this, instance, func_args);
}
...
func_proto_call(js_val *instance, js_args *args, eval_state *state)
{
  ...
  return js_call(state->ctx, this, instance, args);
}
