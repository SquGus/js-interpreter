...
cmp_js(js_prop *a, js_prop *b)
{
  ...
  args_append(args, a->ptr);
  args_append(args, b->ptr);
  js_val *result = js_call(js_cmp_state->ctx, JSUNDEF(), js_cmp_func, args);
...
arr_new(js_val *instance, js_args *args, eval_state *state)
{
  ...
  if (ARGLEN(args) == 1 && IS_NUM(ARG(args, 0))) {
    ...
    if (len < 0 || len >= ULONG_MAX || fmod(len, 1) != 0)
      js_throw(state, js_new_error(E_RANGE, "Invalid array length"));
  ...
  for (i = 0; i < ARGLEN(args); i++)
    js_set(arr, JSNUMKEY(i)->string.ptr, ARG(args, i));
...
arr_proto_pop(js_val *instance, js_args *args, eval_state *state)
{
  ...
  js_val *popped = js_get(instance, key->string.ptr);

  js_del_prop(instance, key->string.ptr);
  js_set_len(instance, len - 1);
  return popped;
}
...
{
  unsigned long len = instance->object.length;
  unsigned nargs = ARGLEN(args);
  unsigned i;
  for (i = 0; i < nargs; i++) {
    js_val *key = JSNUMKEY(len);
    js_set(instance, key->string.ptr, ARG(args, i));
    len++;
  }

  js_set_len(instance, len);
  return JSNUM(len);
}
...
arr_proto_reverse(js_val *instance, js_args *args, eval_state *state)
{
  ...
  while (i < j) {
    ik = JSNUMKEY(i++)->string.ptr;
    jk = JSNUMKEY(j--)->string.ptr;

    iprop = js_get_prop(instance, ik);
    jprop = js_get_prop(instance, jk);
    ...
    if (!iprop && jprop) {
      js_set_prop(instance, ik, jprop->ptr, P_DEFAULT);
      js_del_prop(instance, jk);
    }
    else if (!jprop && iprop) {
      js_set_prop(instance, jk, iprop->ptr, P_DEFAULT);
      js_del_prop(instance, ik);
    }
    else if (jprop && iprop) {
      tmp = iprop->ptr;
      iprop->ptr = jprop->ptr;
...
arr_proto_shift(js_val *instance, js_args *args, eval_state *state)
{
  ...
  js_val *shifted = js_get(instance, "0");
  js_del_prop(instance, "0");

  unsigned long i;
  for (i = 1; i < len; i++) {
    js_val *old_key = JSNUMKEY(i);
    js_val *new_key = JSNUMKEY(i-1);
    js_prop *prop = js_get_prop(instance, old_key->string.ptr);
    js_del_prop(instance, old_key->string.ptr);
    js_set(instance, new_key->string.ptr, prop->ptr);
  }
...
arr_proto_sort(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (i = 0; i < len; i++) {
    prop = js_get_prop(instance, JSNUMKEY(i)->string.ptr);
    if (!prop) {
      i--;
      continue;
    }
...
arr_proto_splice(js_val *instance, js_args *args, eval_state *state)
{
  ...
  unsigned int  args_length = ARGLEN(args);        // number of args
  ...
  while (i < len) {
    ...
    if (i == splice_ind) {
      ...
      while (args_ind < args_length) {
        js_set(keepers, JSNUMKEY(k)->string.ptr, ARG(args, args_ind));
        args_ind++;
        k++;
      }
      // Add the spliced region to the rejects, skip over those indices.
      while (splice_len > 0) {
        val = js_get(instance, JSNUMKEY(i)->string.ptr);
        js_set(rejects, JSNUMKEY(j)->string.ptr, val);
        splice_len--, i++, j++;
      }
      ...
      val = js_get(instance, JSNUMKEY(i)->string.ptr);
      js_set(keepers, JSNUMKEY(k)->string.ptr, val);
      k++, i++;
    }
  }
  ...
  js_set_len(instance, k);
  js_set_len(rejects, j);
  return rejects;
}
...
arr_proto_unshift(js_val *instance, js_args *args, eval_state *state)
{
  ...
  // Add the args
  for (; i < nargs; i++) {
    js_set(newarr, JSNUMKEY(i)->string.ptr, ARG(args, i));
  }

  // Add the instance's elements
  js_val *val;
  for (; j < len; j++, i++) {
    val = js_get(instance, JSNUMKEY(j)->string.ptr);
    js_set(newarr, JSNUMKEY(i)->string.ptr, val);
  }
...
arr_proto_concat(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (; i < len; i++) {
    key = JSNUMKEY(i);
    js_set(concat, key->string.ptr, js_get(instance, key->string.ptr));
  }
  ...
  for (; j < nargs; j++, i++) {
    ...
    if (IS_ARR(arg)) {
      ...
      for (k = 0; k < arg->object.length; k++) {
        ...
        js_set(concat, key->string.ptr, js_get(arg, inner_key->string.ptr));
        // Let the outer loop increment i if we're at the end:
        if (k < arg->object.length - 1) i++;
      }
    }
    else {
      js_set(concat, key->string.ptr, arg);
    }
  }
...
do_join(js_val *arr, js_val *sep)
{
  ...
  for (i = 0; i < arr->object.length; i++) {
    el = js_get(arr, JSNUMKEY(i)->string.ptr);

    if (!first)
      result = JSSTR(js_str_concat(result->string.ptr, sep->string.ptr));
    else
      first = false;

    strval = IS_UNDEF(el) || IS_NULL(el) ? JSSTR("") : TO_STR(el);
    result = JSSTR(js_str_concat(result->string.ptr, strval->string.ptr));
  }
...
arr_proto_slice(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (i = 0; j < k && j < len; j++, i++) {
    val = js_get(instance, JSNUMKEY(j)->string.ptr);
    js_set(slice, JSNUMKEY(i)->string.ptr, val);
  }
...
arr_proto_index_of(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (; i < len; i++) {
    ...
    equals = js_eq(js_get(instance, key->string.ptr), search, true);
...
arr_proto_filter(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (; i < len; i++) {
    ikey = JSNUMKEY(i);
    val = js_get(instance, ikey->string.ptr);
    cbargs = args_new();
    args_append(cbargs, val);
    args_append(cbargs, JSNUM(i));
    args_append(cbargs, instance);
    result = js_call(state->ctx, this, callback, cbargs);
    if (TO_BOOL(result)->boolean.val) {
      jkey = JSNUMKEY(j++);
      js_set(filtered, jkey->string.ptr, js_get(instance, ikey->string.ptr));
    }
  }

  js_set_len(filtered, j);
  return filtered;
}
...
arr_proto_for_each(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (i = 0; i < len; i++) {
    key = JSNUMKEY(i);
    val = js_get(instance, key->string.ptr);
    cbargs = args_new();
    args_append(cbargs, val);
    args_append(cbargs, JSNUM(i));
    args_append(cbargs, instance);
    js_call(state->ctx, this, callback, cbargs);
  }
...
arr_proto_map(js_val *instance, js_args *args, eval_state *state)
{
  ...
  for (i = 0; i < len; i++) {
    ...
    args_append(cbargs, instance);
    result = js_call(state->ctx, this, callback, cbargs);
    js_set(map, key->string.ptr, result);
  }
...
arr_proto_reduce(js_val *instance, js_args *args, eval_state *state)
{
  js_val *callback = ARG(args, 0);
  if (!IS_FUNC(callback))
    js_throw(state, js_new_error(E_TYPE, "%s is not a function", js_typeof(callback)));
  ...
  if (IS_UNDEF(reduction)) {
    if (len == 0)
      js_throw(state, js_new_error(E_RANGE, "Reduce of empty array with no initial value"));

    reduction = js_get(instance, "0");
    i = 1;
  }
  ...
  for (; i < len; i++) {
    ...
    val = js_get(instance, key->string.ptr);
    cbargs = args_new();
    args_append(cbargs, reduction);
    args_append(cbargs, val);
    args_append(cbargs, JSNUM(i));
    reduction = js_call(state->ctx, JSUNDEF(), callback, cbargs);
  }
...
arr_proto_reduce_right(js_val *instance, js_args *args, eval_state *state)
{
  ...
  if (IS_UNDEF(reduction)) {
    ...
    reduction = js_get(instance, JSNUMKEY(i)->string.ptr);
...
bootstrap_array()
{
  ...
  js_attach_prototype(proto, js->function_proto);
  js->array_proto = proto;
