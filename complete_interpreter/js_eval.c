...
static js_val *eval_each(js_val *ctx, ast_node *node) {
  js_val *result = JSUNDEF();
  node_rewind(node);
  while (!node->visited) {
    result = js_eval(ctx, node_pop(node));
  }
  return result;
}

static js_val *str_from_node(js_val *ctx, ast_node *node) {
  ...
  return TO_STR(js_eval(ctx, node));
}
...
static js_val *member_parent(js_val *ctx, ast_node *member) {
  return member->e2->type == NODE_MEMBER ?
    member_exp(ctx, member->e2) :
    js_eval(ctx, member->e2);
}

static js_val *member_child(js_val *ctx, ast_node *member) {
  // In `x.foo` we'll take 'foo' literally, in `x[foo]` we need to eval 'foo'.
  // This distinction is stored as 0/1 in the val slot.
  return member->val ?
    TO_STR(js_eval(ctx, member->e1)) :
    str_from_node(ctx, member->e1);
}

static js_val *member_exp(js_val *ctx, ast_node *member) {
  ...
  return js_get_proto(parent, child_name->string.ptr);
}

static js_val *ident(js_val *ctx, ast_node *id) {
  js_prop *prop = js_get_prop_rec(ctx, id->sval);
  if (!prop) {
    eval_state *state = js_new_state(id->line, id->column);
    js_push_state(state);
    js_throw(state, js_new_error(E_REFERENCE, "%s is not defined", id->sval));
  }
  return prop->ptr;
}

static void put(js_val *ctx, ast_node *ref, js_val *val) {
  char *key;
...
static js_val *add_op(js_val *a, js_val *b) {
  a = js_to_primitive(a, T_NUMBER);
  b = js_to_primitive(b, T_NUMBER);

  if (IS_STR(a) || IS_STR(b))
    return JSSTR(js_str_concat(TO_STR(a)->string.ptr, TO_STR(b)->string.ptr));
...
static js_val *sub_op(js_val *a, js_val *b) {
  // a - b  <==>  a + (-b)
  return add_op(TO_NUM(a), JSNUM(-1 * TO_NUM(b)->number.val));
}

static js_val *mul_op(js_val *a, js_val *b) {
...
static js_val *div_op(js_val *a, js_val *b) {
  a = TO_NUM(a), b = TO_NUM(b);
...
static js_val *mod_op(js_val *a, js_val *b) {
  if (T_BOTH(a, b, T_NUMBER)) {
    if (a->number.is_nan || b->number.is_nan) return JSNAN();
    if (a->number.is_inf || b->number.is_inf) return JSNAN();
...
static js_val *eq_op(js_val *a, js_val *b, bool strict) {
  ...
  if ((IS_STR(a) || IS_NUM(a)) && IS_OBJ(b))
    return eq_op(a, js_to_primitive(b, T_NUMBER), strict);
  if (IS_OBJ(a) && (IS_STR(b) || IS_NUM(b)))
    return eq_op(js_to_primitive(a, T_NUMBER), b, strict);

  return JSBOOL(0);
}

js_val *js_eq(js_val *a, js_val *b, bool strict) {
  return eq_op(a, b, strict);
}

static js_val *neq_op(js_val *a, js_val *b, bool strict) {
  // Invert the result of eq_op
  return JSBOOL(!eq_op(a, b, strict)->boolean.val);
}

static js_val *abstr_rel_comp(js_val *a, js_val *b, bool left_first) {
...
static js_val *lt_op(js_val *a, js_val *b, bool or_equal) {
  js_val *res;
  if (or_equal) {
...
static js_val *gt_op(js_val *a, js_val *b, bool or_equal) {
  js_val *res;
  if (or_equal) {
    res = abstr_rel_comp(a, b, true);
    return JSBOOL(!(IS_UNDEF(res) || res->boolean.val));
...
static js_val *and_exp(js_val *ctx, ast_node *a, ast_node *b) {
  // && operator returns the first false value, or the second true value.
  js_val *aval = js_eval(ctx, a);
  if (TO_BOOL(aval)->boolean.val) return js_eval(ctx, b);
  return aval;
}

static js_val *or_exp(js_val *ctx, ast_node *a, ast_node *b) {
  // || returns the first true value, or the second false value.
  js_val *aval = js_eval(ctx, a);
  if (TO_BOOL(aval)->boolean.val) return aval;
  return js_eval(ctx, b);
}

static js_val *delete_op(js_val *ctx, ast_node *node) {
  ...
  js_prop *prop = js_get_prop(env, name->string.ptr);
  if (!prop->configurable)
    return JSBOOL(0);
  return JSBOOL(js_del_prop(env, name->string.ptr));
}
...
static void assign(js_val *obj, char *name, js_val *val, char *op) {
  if (STREQ(op, "=")) {
    js_set_rec(obj, name, val);
    return;
  }

  // Handle other assignment operators
  js_val *cur = js_get_rec(obj, name);
  if (STREQ(op, "+=")) js_set_rec(obj, name, add_op(cur, val));
  if (STREQ(op, "-=")) js_set_rec(obj, name, sub_op(cur, val));
  if (STREQ(op, "*=")) js_set_rec(obj, name, mul_op(cur, val));
  if (STREQ(op, "/=")) js_set_rec(obj, name, div_op(cur, val));
}

static js_val *assign_exp(js_val *ctx, ast_node *node) {
  js_val *val = js_eval(ctx, node->e2);
...
static js_val *var_dec(js_val *ctx, ast_node *node, bool ignore_rval) {
  // If node->val is set, the variable declaration has already been hoisted and
  // should not be touched.
  if (!node->val) {
    if (node->e2 == NULL || ignore_rval)
      js_set_prop(ctx, node->e1->sval, JSUNDEF(), P_WRITE | P_ENUM);
    else
      js_set_prop(ctx, node->e1->sval, js_eval(ctx, node->e2), P_WRITE | P_ENUM);
  }
...
static js_val *return_stmt(js_val *ctx, ast_node *node) {
  js_val *result = node->e1 ? js_eval(ctx, node->e1) : JSUNDEF();
  if (IS_FUNC(result))
...
static js_val *break_stmt() {
  js_val *signal = JSNULL();
  signal->signal = S_BREAK;
  return signal;
}

static js_val *if_stmt(js_val *ctx, ast_node *node) {
  if (TO_BOOL(js_eval(ctx, node->e1))->boolean.val)
    return js_eval(ctx, node->e2);
  else if (node->e3 != NULL)
    return js_eval(ctx, node->e3);
  return JSUNDEF();
}

static js_val *switch_stmt(js_val *ctx, ast_node *node) {
  js_val *val, *result, *test = js_eval(ctx, node->e1);
  ...
  if (defaultclause && !matched) {
    result = js_eval(ctx, defaultclause->e2);
    if (result->signal == S_BREAK)
      result->signal = S_NONE;
    return result;
  }
...
static void func_dec_scan(js_val *ctx, ast_node *node) {
  ...
  while (!node->visited) {
    ...
    if (child->type == NODE_FUNC) {
      ...
      js_set_prop(ctx, name, JSFUNC(child), P_WRITE | P_ENUM);
    }
  }
  node_rewind(node);
}

static void var_dec_scan(js_val *ctx, ast_node *node) {
...
static js_val *stmt_lst(js_val *ctx, ast_node *node) {
  js_val *result = NULL;
  ast_node *child;
  node_rewind(node);
  while (!node->visited) {
...
static js_val *src_lst(js_val *ctx, ast_node *node) {
...
static js_val *obj_lit(js_val *ctx, ast_node *node) {
  js_val *obj = JSOBJ();
  obj->object.parent = ctx;
  if (node->e1 != NULL) js_eval(obj, node->e1);
  return obj;
}

static js_val *arr_lit(js_val *ctx, ast_node *node) {
  js_val *arr = JSARR();
  if (node->e1 != NULL) {
    int i = 0;
    while (!node->e1->visited) {
      js_set(arr, JSNUMKEY(i++)->string.ptr, js_eval(ctx, node_pop(node->e1)));
    }
    js_set_len(arr, i);
  }
  return arr;
...
static void while_stmt(js_val *ctx, ast_node *cnd, ast_node *stmt) {
  js_val *result;

  while (TO_BOOL(js_eval(ctx, cnd))->boolean.val) {
    result = js_eval(ctx, stmt);
    if (result->signal == S_BREAK) break;
  }
}

static void for_stmt(js_val *ctx, ast_node *exp_grp, ast_node *stmt) {
  js_val *result;

  if (exp_grp->e1)
    js_eval(ctx, exp_grp->e1);

  while (TO_BOOL(exp_grp->e2 ? js_eval(ctx, exp_grp->e2) : JSBOOL(1))->boolean.val) {
    result = js_eval(ctx, stmt);
    if (result->signal == S_BREAK) break;
    if (exp_grp->e3)
      js_eval(ctx, exp_grp->e3);
  }
}

static void forin_stmt(js_val *ctx, ast_node *node) {
  js_val *result, *obj, *env, *name;

  obj = js_eval(ctx, node->e2);
  env = ctx;
  ...
  while (obj != NULL) {
    OBJ_ITER(obj, p) {
      if (p->enumerable) {
        ...
        js_set_rec(env, name->string.ptr, JSSTR(p->name));
        result = js_eval(ctx, node->e3);
        if (result->signal == S_BREAK) break;
      }
    }
    obj = obj->proto;
...
static js_val *try_stmt(js_val *ctx, ast_node *node) {
  eval_state *state = js_new_state(node->line, node->column);
  state->catch = true;
  state->ctx = ctx;
  js_push_state(state);
  ...
  // Catch
    ...
    js_set(ctx, node->e2->e1->sval, js_get(ctx, "JS_LAST_ERROR"));
    js_eval(ctx, node->e2->e2);
  }

  // Finally
  if (node->e3 && node->e3->e1)
    js_eval(ctx, node->e3->e1);

  return JSUNDEF();
}

static js_val *throw_stmt(js_val *ctx, ast_node *exp) {
  eval_state *state = js_new_state(exp->line, exp->column);
  js_push_state(state);
  js_val *val = js_eval(ctx, exp);
  js_throw(state, val);
  return JSUNDEF();
}
...
static js_val *setup_call_env(js_val *ctx, js_val *this, js_val *func, js_args *args) {
  ...
  js_set(scope, "this", this);
  js_set(scope, "arguments", arguments);
  ...
  if (func_node->sval != NULL)
    js_set(scope, func_node->sval, func);

  // Set up the (array-like) arguments object.
  unsigned long i, arglen = ARGLEN(args);
  for (i = 0; i < arglen; i++)
    js_set(arguments, JSNUMKEY(i)->string.ptr, ARG(args, i));
  js_set_class(arguments, "Arguments");
  js_set(arguments, "callee", func);
  js_set(arguments, "length", JSNUM(arglen));
  ...
  if (func_node->e1 != NULL) {
    ...
    while (!params->visited) {
      if (args->arg) {
        js_set(scope, node_pop(params)->sval, args->arg);
        if (args->next)
          args = args->next;
        else
          args->arg = NULL;
        ...
        js_set(scope, node_pop(params)->sval, JSUNDEF());
      }
    }
  }
  return scope;
...
static js_args *build_args(js_val *ctx, ast_node *args_node) {
  js_args *args = args_new();

  if (args_node->e1 == NULL) return args;
  if (!args_node->visited) {
    args->arg = js_eval(ctx, node_pop(args_node));
...
static js_val *call(js_val *ctx, js_val *this, js_val *func, eval_state *state, js_args *args) {
  ...
  return js_eval(func_scope, func->object.node->e2);
}

static js_val *call_exp(js_val *ctx, ast_node *node) {
  js_val *maybe_func = js_eval(ctx, node->e1);

  // Special treatment for:
  //   CallExpression [ Expression ]
  //   CallExpression . Identifier
  if (node->e2->type != NODE_ARG_LST)
    return js_get_proto(maybe_func, str_from_node(ctx, node->e2)->string.ptr);

  eval_state *state = js_new_state(node->line, node->column);

  state->ctx = ctx;
  if (!IS_FUNC(maybe_func))
    js_throw(state, js_new_error(E_TYPE, "%s is not a function", js_typeof(maybe_func)));
  js_args *args = build_args(ctx, node->e2);

  // Check for a bound this (see Function#bind)
  js_val *this = maybe_func->object.bound_this ?
    maybe_func->object.bound_this : js_get(ctx, "this");
...
static js_val *new_exp(js_val *ctx, ast_node *exp) {
  js_val *ctr;
  js_args *args;

  // new F(x, y, z)
  if (exp->e1 && exp->e1->type == NODE_MEMBER) {
    ctr = js_eval(ctx, exp->e1->e2);
    args = build_args(ctx, exp->e1->e1);
  }
  // new F
  else {
    ctr = js_eval(ctx, exp->e1);
    args = args_new();
  }
  ...
  if (!IS_FUNC(ctr))
    js_throw(state, js_new_error(E_TYPE, "%s is not a function", js_typeof(ctr)));

  js_val *res, *obj = JSOBJ(), *proto = js_get(ctr, "prototype");
  ...
  if (IS_OBJ(proto) && res->proto == js->object_proto)
    res->proto = IS_OBJ(proto) ? proto : js->object_proto;
...
static js_val *postfix_exp(js_val *ctx, ast_node *node) {
  js_val *old_val = TO_NUM(js_eval(ctx, node->e1));
  char *op = node->sval;
  if (STREQ(op, "++")) {
...
static js_val *prefix_exp(js_val *ctx, ast_node *node) {
  char *op = node->sval;
  ...
  if (STREQ(op, "typeof")) {
    if (node->e1->type == NODE_IDENT)
      return JSSTR(js_typeof(js_get_rec(ctx, node->e1->sval)));
    return JSSTR(js_typeof(js_eval(ctx, node->e1)));
  }
  if (STREQ(op, "void")) {
    js_eval(ctx, node->e1);
    return JSUNDEF();
  ...
  if (STREQ(op, "+"))
    return TO_NUM(js_eval(ctx, node->e1));
  if (STREQ(op, "!"))
    return JSBOOL(!TO_BOOL(js_eval(ctx, node->e1))->boolean.val);
  if (STREQ(op, "-")) {
    js_val *x = TO_NUM(js_eval(ctx, node->e1));
...
static js_val *bin_exp(js_val *ctx, ast_node *node) {
  char *op = node->sval;
  ...
  js_val *a = js_eval(ctx, node->e1);
  js_val *b = js_eval(ctx, node->e2);
  ...
  if (STREQ(op, "instanceof")) {
    if (!IS_FUNC(b)) {
      ...
      js_throw(NULL, js_new_error(E_TYPE, fmt, js_typeof(b)));
    }
    return js_has_instance(b, a);
  }
  if (STREQ(op, "in")) {
    if (!IS_OBJ(b)) {
    ...
    return js_has_property(b, TO_STR(a)->string.ptr);
  }

  int a_int32 = js_to_int32(a)->number.val;
  int b_int32 = js_to_int32(b)->number.val;
  ...
  unsigned a_uint32 = js_to_uint32(a)->number.val;
  unsigned b_uint32 = js_to_uint32(b)->number.val;
...
static js_val *base_exp(js_val *ctx, ast_node *node) {
...
js_val *js_eval(js_val *ctx, ast_node *node)
{
  ...
  switch (node->type) {
    ...
    case NODE_BLOCK:       return js_eval(ctx, node->e1);
    case NODE_EXP_STMT:    return js_eval(ctx, node->e1);
    case NODE_VAR_STMT:    return js_eval(ctx, node->e1);
    case NODE_VAR_DEC_LST: return eval_each(ctx, node);
    case NODE_PROP_LST:    return eval_each(ctx, node);

    case NODE_PROP:        js_set(ctx, node->e1->sval, js_eval(ctx, node->e2)); break;
    ...
    default:
    {
      eval_state *state = js_new_state(node->line, node->column);
      js_push_state(state);
      js_throw(state, js_new_error(E_SYNTAX, "Unsupported syntax type (%d)", node->type));
    }
  }
